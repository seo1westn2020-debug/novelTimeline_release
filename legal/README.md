# Legal Documents · 법적 문서 인덱스

novel-timeline을 사용·구매하실 때 적용되는 이용약관, 환불 정책, 개인정보 처리방침입니다.

Legal terms that apply when you use or purchase novel-timeline.

| 문서 / Document | 한국어 | English |
|---|---|---|
| 이용약관 / Terms of Service | [terms.md](./terms.md) | [terms.en.md](./terms.en.md) |
| 환불 정책 / Refund Policy | [refund.md](./refund.md) | [refund.en.md](./refund.en.md) |
| 개인정보 처리방침 / Privacy Policy | [privacy.md](./privacy.md) | [privacy.en.md](./privacy.en.md) |

---

## Governing version · 적용되는 원본

- **English versions are the legally binding originals.** In case of any discrepancy between the Korean and English texts, the English text prevails.
- **영문본이 법적 원본입니다.** 한국어본과 영문본 사이에 차이가 있을 경우 **영문본이 우선**합니다. 한국어본은 한국어 사용자의 편의를 위한 번역입니다.

## Effective date · 시행일

- Initial publication / 최초 공표: **2026-04-19**
- Last updated / 최종 갱신: **2026-04-21** (영구 라이선스 조항 제거, 3개월 체험 + 읽기 전용 모드 도입)

## Contact

- Email: `askNovelTimeline@gmail.com`
- GitHub: https://github.com/seo1westn2020/novelTimeline_release

Changes to these documents are announced through **GitHub Releases** and the application's **Settings → About** section.

이 문서의 변경은 **GitHub Releases**와 앱 내 **설정 → 정보** 섹션을 통해 공지됩니다.
